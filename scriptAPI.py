...
try:
    ...
    def obter_filmes_tmdb(num_filmes):
        ...
        while len(filmes) < num_filmes:
            url = f'https://api.themoviedb.org/3/discover/movie?api_key={
                api_key}&language=pt-BR&sort_by=popularity.desc&include_adult=false&include_video=false&page={pagina}'
    ...
    def obter_atores_tmdb(num_atores):
        ...
        while len(atores) < num_atores:
            url = f'https://api.themoviedb.org/3/person/popular?api_key={
                api_key}&language=pt-BR&page={pagina}'
            response = requests.get(url)
            data = response.json()
            if 'results' in data:
    ...
    def popular_tabela_usuario(num_registros):
        ...
        for i in range(num_registros['usuario']):
            ...
            try:
                ...
                print(f"Erro ao inserir registro de usuário com ID {
                      next_id}: {err}")
            next_id += 1
